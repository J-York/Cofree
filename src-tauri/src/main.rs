                            ...
                            if let Some(delta) = choice.get("delta") {
                                ...
                                if let Some(tc) = delta.get("tool_calls").and_then(Value::as_array) {
                                    for tool_call_delta in tc {
                                        ...
                                        let tc_entry = ensure_tool_call_entry(&mut tool_calls_json, tc_index);
                ...
                if let Ok(parsed) = serde_json::from_str::<Value>(data) {
                    ...
                    match event_type {
                        ...
                        "response.function_call_arguments.delta" | "response.function_call_arguments.done" => {
                            ...
                            append_tool_call_delta(
                                entry,
                                parsed.get("call_id").and_then(Value::as_str).or_else(|| Some(key.as_str())),
                        ...
                        "response.output_item.added" | "response.output_item.done" => {
                            if let Some(item) = parsed.get("item") {
                                if item.get("type").and_then(Value::as_str) == Some("function_call") {
                                    ...
                                    append_tool_call_delta(
                                        entry,
                                        item.get("call_id").and_then(Value::as_str).or_else(|| Some(key.as_str())),
                        ...
                        "content_block_start" => {
                            ...
                            if let Some(content_block) = parsed.get("content_block") {
                                if content_block.get("type").and_then(Value::as_str) == Some("tool_use") {
                                    let entry = ensure_tool_call_entry(&mut tool_calls_json, block_index);
                                    append_tool_call_delta(
                                        entry,
                                ...
                                match delta.get("type").and_then(Value::as_str).unwrap_or_default() {
                                    "text_delta" => {
                                        if let Some(text) = delta.get("text").and_then(Value::as_str) {
                                            ...
                                            emit_stream_chunk_event(app, request_id, text, false, None);
                                        }
                                    }
                                    "input_json_delta" => {
                                        if let Some(partial_json) = delta.get("partial_json").and_then(Value::as_str) {
                                            let entry = ensure_tool_call_entry(&mut tool_calls_json, block_index);
                                            append_tool_call_delta(entry, None, None, Some(partial_json));
                                        }
                                    }
                                    _ => {}
                                }
...
async fn post_litellm_chat_completions_stream(
    ...
    for (index, endpoint) in endpoints.iter().enumerate() {
        ...
        let synthetic_body = match normalize_protocol(&protocol) {
            ...
            "anthropic-messages" => parse_anthropic_messages_stream(&app, response, &request_id).await,

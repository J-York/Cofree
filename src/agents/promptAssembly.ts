...
const RULE_CORE = [
  ...
  "4) **Todo 一致性**：当任务已经被拆解为 todo 时，一次只推进一个步骤。开始推进、完成、阻塞或失败时，使用 update_plan 明确更新状态，而不是只在自然语言里描述。回复用户前，确保所有**已经实质完成的内部步骤**都已通过 update_plan 标记为 completed，但**如果某个步骤需要等待用户确认或提供信息，请保持其 pending 状态或打上 blocked 标记**，绝对不要把等待用户的步骤标记为 completed。",
].join("\n");

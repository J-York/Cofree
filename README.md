...
- 内置「通用 Agent」角色，可阅读代码、提出编辑、执行命令；用户可在设置页自定义 system prompt、工具权限、模型绑定派生新的 Agent。
...
| `check_shell_job` | `auto` | 查询后台 shell 任务状态 |
...
| `update_plan` | 内部 | 维护 TODO 计划，无工作区副作用 |
| `ask_user` | 内部 | 暂停执行并向用户提问 |
...
3. 按需调整工具权限，或自定义 Agent 的 system prompt 与模型绑定。
...
发布流通过 GitHub Actions 自动化：推送形如 `v0.1.0`（或任意 `vX.Y.Z`）的 tag 即可触发多平台构建矩阵（macOS Apple Silicon、macOS Intel、Windows x64）。若配置了 updater 签名密钥，还会额外上传 `latest.json` 与 updater 签名工件。
